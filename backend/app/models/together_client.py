"""Together AI client — hosted open-source models via an OpenAI-compatible API.

One API key unlocks Together's catalog (Llama, DeepSeek, Qwen, Kimi, and more)
at open-source pricing. The request/response shape, auth, and SSE streaming
format mirror OpenRouter's exactly (both sit on an OpenAI-compatible serving
stack), so this reuses the same wire helpers and tool-call fragment
accumulation logic as `openrouter_client.py`.
"""
...
class TogetherClient(ModelClient):
    ...
    async def stream_chat(
        ...
        if not self.api_key:
            raise RuntimeError(
                "TOGETHER_API_KEY is not set. Add it to .env or route this "
                "agent to the 'ollama' or 'mock' provider."
            )

        headers = {"Authorization": f"Bearer {self.api_key}"}
        payload = {
            "model": model or settings.together_default_model,
            "messages": [to_openai_wire(m) for m in messages],
            "stream": True,
            # Standard OpenAI-spec way to get a final usage chunk on a
            # streamed response (distinct from OpenRouter's own `usage.include`
            # extension). If Together doesn't honour it, the field is simply
            # ignored and usage falls back to the estimate below — never fatal.
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(300.0), transport=self._transport) as client:
            async with client.stream(
                ...
                async for line in resp.aiter_lines():
                    ...
                    # A trailing event (choices: []) carries the final usage.
        ...
        self.last_usage = Usage(
            ...
            cost_usd=estimate_cost(
                model or settings.together_default_model,
                prompt_tokens,
                completion_tokens,
            ),
        )
